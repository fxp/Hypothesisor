...
async function refreshJobs() {
  ...
  const mine = all.filter((j) => j.type === "reformat");
...
function renderJobItem(job) {
  ...
  item.className = `job-item reformat ${statusClass} ${actionable ? "actionable" : ""}`.trim();
  item.dataset.jobId = job.id;

  const title = job.reformatTitle || job.sourceTitle || job.sourceHostname || "(untitled)";
  ...
  if (actionable && job.status === "done" && job.reformatId) {
...
function formatError(e) {
  if (!e) return "";
  const ctxLabel = e.ctx === "bigmodel" ? t("ctx_bigmodel") : "";
  switch (e.code) {
...
$("generate").addEventListener("click", async () => {
  ...
  try {
    ...
    const { bigmodelKey } = await getSettings();
    if (!bigmodelKey) { setStatus(t("status_need_bigmodel"), "error"); return; }

    const customPrompt = $("formatCustom").value.trim() || null;
